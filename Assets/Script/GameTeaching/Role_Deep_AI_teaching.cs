...
namespace chia
{
    ...
    public class Role_Deep_AI_teaching : Role
    {
            ...
            Node root = new Selector(new List<Node>
        {
            ...
            {   
                ...
                new Leaf("Check_TaskGoToTarget",Check_TaskGoToTarget),//受傷、攻擊時不能移動
        ...
        {
            if (stateInjuried_Insitu || stateInjuriedUp)//受傷時
            {
                        ...
                        if (tempTrasforms[i].name == "Deep_鬼哭斬")
                        {
                            ...
                            if (!stateAttack)//不是攻擊狀態(不然一直按會一直攻擊，攻擊動畫還沒做完就又攻擊)
                            {
                               ...
                               this.gameObject.GetComponentsInChildren<Transform>(true)[i].gameObject.SetActive(true);//開啟Deep_鬼哭斬物件
                                ...
                                //print("攻擊動畫_開" + Time.time);
                                animator.SetTrigger(parSkill01);
                                aud.PlayOneShot(deep_sf2);//音效
                                StartCoroutine(WaitCloseGameObject_AI(tempIndex, node));//等0.5秒關閉GameObject
                            }
                        }
                    }
