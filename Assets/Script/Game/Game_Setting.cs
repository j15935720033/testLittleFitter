...
namespace chia
{
    public class Game_Setting : MonoBehaviour
    {
        ...
        Role[] ScriptRole;//所有Role和Role的子類別
        private void Start()
        {
            //檢查場景上的BGM數量是否<=0
        ...
        private void Awake()
        {
            ScriptRole=FindObjectsOfType<Role>();//找所有Role和子類別的component
        }
        private void Update()
        {
            print("statePause"+statePause);
        }
        /// <summary>
        /// 使用Slider控制音樂
        ...
        public void GamePause()
        {
            ...
            foreach (var scriptRole in ScriptRole)
            {
                scriptRole.enabled = !statePause;//關閉腳本(元件)
            }
